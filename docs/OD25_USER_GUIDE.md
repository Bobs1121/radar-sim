...
用户不填写“OD25”项目名。Windows Agent 使用用户给出的脚本相对位置和脚本内容识别：
...
## 4. Windows 用户需要做的一次性部署

### 4.1 哪些用户需要安装

- `source: build`：必须在代码仓所在 Windows 电脑安装 light Agent；编译和上传完成前保持 Agent 在线。
- `source: existing` 且 Selena、Runtime、数据或配置文件只在 Windows 本地：需要 light Agent，或直接在该 Windows 电脑运行 SDK。
- 所有输入都已在 Linux/Cluster 可访问共享存储：不需要 Windows Agent。
...
当前可信内网测试服务关闭认证，安装器会从健康接口自动识别，因此不要求令牌。以后正式服务器开启认证时，只需在同一命令增加管理员给出的 `-AgentToken` 和 `-ApiToken`。

安装器会创建隔离 Python 环境、保存服务器地址、检查 VS、验证 Agent 能力并启动后台 Agent。以后启动：

```powershell
.\scripts\start_windows.ps1 -Background
```
...
| 现象 | 用户先检查什么 |
|---|---|
| 一直停在“识别配置” | Windows Agent 是否在线；两个脚本和 Runtime 是否确实位于所填电脑 |
| 识别失败 | `code_path` 是否包含两个脚本；OD25 脚本是否为本指南的相对位置 |
