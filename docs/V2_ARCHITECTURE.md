...
> 状态：V2 唯一实施基线，当前外围框架 release 已部署
> 日期：2026-08-20
...
当前部署若关闭 Bearer 认证，只能视为受信内网试用：`X-Rsim-User` 负责 owner 路由，不是身份认证，不能对不受信用户宣称强多租户隔离。正式对外部署必须启用服务端认证并将 owner 从认证主体派生；这不改变 YAML、Connector 或调度合同。
...
本矩阵是发布门禁，不代表所有项目已通过；每次真实验收的 Job ID、Manifest、结果校验和外部依赖必须记录在最新 handoff。
