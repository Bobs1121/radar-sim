...
5. [发布与部署](release-deployment.md)：当前 Linux release、Connector、发布门禁和回滚规则。
6. [Windows Connector](windows-one-click-connector.md)：一次安装、升级、恢复和用户操作。
7. [当前已知边界](KNOWN_ISSUES.md)：只记录仍然存在的产品边界，不记录历史 Job 故障。
8. [当前交接](../HANDOFF.md)：最新生产状态和证据索引。
