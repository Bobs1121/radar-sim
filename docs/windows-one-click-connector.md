...
公开入口只有一个“统一连接组件”。它会按任务自动承担本机路径访问、Selena 编译、数据/产物直传、Cluster 调度准备或本地仿真。公共 API、SDK 和下载入口只接受 `mode=unified`；旧 `light/full` 不再兼容。

这不是“每个任务安装一次”的临时程序：安装成功后会在当前 Windows 用户下保存服务地址和稳定 owner，注册登录自启/断线重连。可信内网的无认证 Web 首次使用会要求输入公司 NTID（内部规范化为 `user-<小写 NTID>`）；SDK 默认使用同一格式的本机登录名，也可显式传入同一个规范化值（如 `user="user-alice"`）。之后同一台电脑上的 Web 和 SDK 任务都复用这条连接，不要求用户反复填写 Agent、服务器地址或路径绑定。清理浏览器缓存或换浏览器时，只需再次输入同一 NTID，不必重装 Connector；如果本机仍保存旧的 `web-*`/`sdk-*` 临时 owner，页面会保留旧身份读取并提示一次升级，运行新入口会原地迁移 owner、保留 Agent ID 和路径绑定。旧 owner 下已完成任务不会被静默改写，升级前请先导出仍需查看的历史记录。只有换电脑、换 Linux 服务地址或主动卸载时才需要重新连接；新任务的代码/数据路径由任务 YAML 提供，连接组件自动做一次性授权和健康检查。

当服务端升级了不兼容的任务合同，能力接口会返回 `windows_connector_update_required`。旧组件此时不会领取新任务，也不会被显示成普通“正在重连”。用户只需在当前 Web 点击“一键更新本机组件”，或由 SDK 重新调用 `download_windows_connector_for_run()` 并运行下载入口；安装程序原地更新代码，保留既有 Agent ID、owner 配对、自启动任务与路径绑定，不需要卸载或重新填写 YAML。普通补丁不提升契约版本，因此不会要求用户频繁更新。

Web 在任意页面顶部显示全局更新横幅，不要求用户先提交一个失败任务才发现版本过旧。用户点击“一键更新本机组件”、双击浏览器下载的同源连接程序后，页面通过能力轮询自动确认新组件上线并隐藏横幅。浏览器安全策略不允许网页静默执行本地程序，因此“下载后双击运行”是唯一保留的用户动作；服务端不会远程静默替换用户电脑上的程序。
...
- 本 Sprint 按产品决定关闭登录，`scripts/linux_deploy.sh` 默认使用 `--insecure-no-auth`，只允许部署在受信内网。`X-Rsim-User`/稳定 NTID 只是可伪造的隔离标签，不是安全认证；不要把该入口暴露到不受信网络。可设置 `RSIM_INSECURE_NO_AUTH=0` 恢复 Bearer 认证。
