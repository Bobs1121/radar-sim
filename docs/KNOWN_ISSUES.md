...
> 更新时间：2026-08-18。实时阻断以 [`../HANDOFF.md`](../HANDOFF.md) 和最新修复 handoff 为准，不沿用历史任务状态。
...
5. Cluster 结果收集已按数据集数量动态扩大扫描并保留完整 per-input manifest；结果归档已接入 retention、过期 GC 和磁盘水位。仍需对接分页展示并做真实超大批量/低磁盘压力验收，不能把 UI 分页上限当成仿真完成判断。
