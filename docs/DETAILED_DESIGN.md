...
- 当前发布版只接受已登记 Runtime Bundle，不接受裸 `Selena.exe` 路径自动导入。
- Windows 安装当前为一键脚本与后台启动，不包含操作系统服务/自动更新；这不影响前台和后台 Agent 使用。
...
- 任何自动化分支编译路径在接入前必须证明不 checkout/stash/reset/force 用户主工作区。
...
| Git operations | Git CLI via `core/repo.py` | 保留 Git CLI，分支自动编译只允许 detached worktree；禁止引入会隐藏 checkout 行为的抽象层 |
...
- `current workspace/branch build + cluster`：可路由到 Windows full 或用户授权的 Windows 轻量 Agent；Agent 完成编译、产物/数据 staging 后必须交还中央调度；
...
1. `current_workspace`
   ...
   - 使用绑定工作区真实内容，包括未提交修改。
2. `branch`
   - 必须有 Windows `build.selena` + `source.git.worktree`；
   - 解析本地/远端 ref 后创建隔离 worktree；
   - 不允许 checkout 用户主工作区。
3. `existing`
...
4. `auto`
   - 先寻找兼容已有产物；
   - 有可用 Windows 构建节点时，可给出当前工作区/分支构建建议；
...
| branch build | local | Windows full worktree/build + local sim | 只允许 Windows full deployment |
| branch build | cluster | Windows full 或 light Agent worktree/build/upload + platform cluster | Linux 不编译；light Agent 不运行仿真 |
...
对已有 Selena，`prepare_source` 和 `build_selena` 标记为 `skipped`；对共享数据，上传子步骤标记为 `skipped`。首版可按顺序执行，后续再并行 Source 和 Data，不改变模型。

### 7.2 幂等与重试

- `resolve_spec`：同一 spec + catalog revision 可重放；
- `prepare_source`：worktree 以 job/stage ID 命名，可检测并恢复；
...
1. `git fetch` 所需 remote/ref；
2. 解析唯一 commit；
3. `git worktree add --detach <job-dir> <commit>`；
4. 在 worktree 内执行构建；
5. 登记产物；
6. 在 finally 中执行 `git worktree remove` 和 `git worktree prune`。
...
现有 `core/repo.py::prepare_repo_worktree` 可作为起点，但必须接入统一 Build Stage；旧的 `prepare_repo_context` 不再用于自动分支构建。
...
| `core/repo.py` | Source adapter | worktree-first；停用破坏性 checkout 路径 |
...
- 给历史文档标记“非当前产品口径”；
- 修复或隔离会破坏用户工作区的自动 checkout 风险；在修复前，Web/API 的分支自动编译不得走旧 `prepare_repo_context()` 路径。
...
- 接入 worktree、dirty fingerprint、Preflight、Manifest 和 progress parser；
...
| Git 分支构建 | detached worktree |
| 当前修改构建 | 当前工作区锁 + 前后 fingerprint |
