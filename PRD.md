...
> 版本：2.0 单轨
> 状态：V2 当前产品基线；Selena 内部结果内容不在外围框架验收范围
> 日期：2026-08-20
...
- 安装完成必须确认当前电脑、当前稳定用户身份和当前 Connector 版本全部一致；不能用同一用户的另一台电脑在线代替本机成功。
- 编译脚本、本地仿真、Cluster 状态查询和结果下载都必须有有限等待、稳定错误和可重试边界；服务/Connector 重启不得重复运行同一 Selena 或重复提交同一 Cluster 仿真。
- 任务成功必须有一致的业务 Manifest 和结果证据；仅进程退出码、网页进度或 Cluster 页面状态不能单独判成功。
