...
3. 用真实成功任务 `job_d8b902defaad` 的本机私有 run 输出做了非夹具验收：`239,051,624` bytes MF4 已写到 `.acceptance/result-root/job_d8b902defaad/outputs/`，文件 SHA-256 为 `c7dd6cc58e9db120ba95bf77f07325a3266660875a299493a68a440da643b6e7`；目录 Manifest 记录 1/1 succeeded 且不含源/目标物理路径。第一次返回 `delivered`，第二次返回 `already_present`，内容校验和均为 `sha256:d90450c830f7bd343f967f44655e9c51a6f5dcef546de3c0501f33e3de29cb40`。随后又用实际安装目录的 Connector v6 Python/`cli.agent._execute_v5_local_collect` 对同一真实 lease 验收，成功写入 `.acceptance/connector-v6-result-root/job_d8b902defaad`，返回 `result_ref=c3a5...e2eb`、`delivery.status=delivered/file_count=1`，证明不是只在仓库测试解释器中可用。
...
- 现场旧 owner `web-19fea3ee69ee9c044e2f4b3e8` 在线能力实测：`configured_count=1/available=false/reconnecting=false`，`windows_connector.update_required=true/outdated_count=1/required_contract_version=2`；用 `job_f4c4eca63c52` 原始 spec 调用无持久化 validate，唯一 Windows blocker 为 `windows_connector_update_required`，动作明确保留 Agent ID、绑定和 YAML。全新审计 owner 为 `configured_count=0/outdated_count=0`，证明过期状态按 owner 隔离。线上 Web 已包含更新状态和按钮。
...
- 持久化修复：新电脑 `10.190.164.73` 在 `03:00:25` 后无任何 `poll/heartbeat/log/result` 请求，且用户确认没有休眠、断网或手工中止，证明原单一计划任务的重复触发没有完成自愈。新版一键安装增加独立 `RadarSimConnector-<user>-Watchdog`，每 2 分钟验证监督进程的 PID 与命令行，缺失时停止假运行实例并重新启动 Connector；Connector 与 Watchdog 分别写入 `%LOCALAPPDATA%\radar-sim\logs\connector.log` 和 `watchdog.log`，各自按大小轮转。
- 验证门禁：多用户/Agent/API/Cluster 定向套件 `134 passed, 1 warning`；全量套件曾在 185 秒超时且无最终汇总，因此不得表述为全量通过。
- 部署：Linux `10.190.171.44:8877` 已更新三项服务端修复并重启。带独立 Watchdog、owner 确认和日志降噪的 Connector ZIP 已覆盖服务端 `dist/rsim-windows-connector.zip`，SHA-256 为 `c7b5959817024d7c105fa07858748e0d94019c4651aa71979bf1bd8e954cbacf`。已安装旧 Connector 不会自动获得 Watchdog，需要在测试阶段从 Web 再执行一次“一键连接”；安装器保留 owner 与已有本地配置，不要求重新填写 YAML。
...
| 日期 | 范围 | 证据 |
|---|---|---|
| 2026-07-24 | Windows 数据准备稳定性与 release `7af55d7` | 现场计划任务此前以 `0xC000013A` 退出，根因是 Windows 默认 `StopIfGoingOnBatteries/DisallowStartIfOnBatteries`；安装器现显式允许电池启动且切换电池不停止。`prepare_data` 的取消检查贯穿数据发现、每个 1MB 哈希块和上传块；本地仿真数据只使用 path/size/mtime 指纹，不再为 19.8GB 本地数据做无意义 SHA256，Cluster 上传仍保留 SHA256 完整性。Job 聚合现在将“上游 succeeded + 取消 Stage”正确收敛为 cancelled，真实 failed Stage 仍优先；API/Web 区分未配置和已配置暂时离线，重连状态仍保留“重新连接本机”兜底，避免陈旧登记把用户锁死。提交 `7af55d7` 已推送；Windows 全量 `1298 passed, 8 skipped`，独立审查 136 passed，最终关键取消/电池/重连门禁 Windows 与 Linux release 均 7 passed。Linux 已切换 `/home/hoz2wx/radar-sim-v1-7af55d7`，service active、PID `1696350`、health 正常、Cluster available。当前 Windows 已无交互复用原 `full + linux`、服务地址和 Agent ID 更新，connector PID `60276`；计划任务 Running，两个电池策略均为 false，服务端 `windows_full available=true,count=1,configured_count=1,reconnecting=false`。生产 heartbeat 为 10 秒；为制造小文件取消窗口而扫描数百 MB/GB 会违背禁大数据门禁，因此本轮以受控慢发现 + 真实 ControlService 终态测试验收，未触发仿真或大数据重扫。live Bundle catalog 共 9 个，没有任何可证明 `feature/CRGVXPFIII-2492...` 来源的 Bundle；仅有 release/2430 和分支证据为空的 `existing` Bundle。下一次真实仿真必须使用明确匹配 feature/2492 的新编译/导入产物，并同时匹配 Runtime 与 MatFilter，禁止再次用 release/2430 盲跑。 |
...
### 2026-07-24 - Windows 稳定性补丁与 feature/2492 下次真实仿真门禁

- Release: 稳定性提交 `7af55d7` 已推送并部署到 `/home/hoz2wx/radar-sim-v1-7af55d7`；切换前 active jobs 为 0。service 现为 active，PID `1696350`，进程 cwd 和 release 一致，health 正常，Cluster available。
- Windows: 一键安装入口复用现有 `full + linux`、`http://10.190.171.44:8877` 和 `agent-HOZ2WX-WX8-C-0001A` 完成后台更新，无需用户重新填写业务配置。connector PID `60276`；计划任务为 Running，`StopIfGoingOnBatteries=False`、`DisallowStartIfOnBatteries=False`；服务端 heartbeat 后 `windows_full available=true,count=1,configured_count=1,reconnecting=false`。
- Verification: Windows 全量 `1298 passed, 8 skipped, 1` 个既有 Starlette deprecation warning；独立只读审查专项 136 passed。发布目录关键 7 项再次 7 passed，覆盖慢速发现的 heartbeat 取消、分块哈希取消、本地数据免内容哈希、cancelled Job 聚合、真实 failed 优先、configured/reconnecting 和电池参数。Linux 扩展 136 项有 1 个既有 Windows Agent 资产授权测试在 POSIX 临时路径失败，关键 7 项和 Linux 运行路径未失败；该跨平台测试夹具问题保留修正，不误报全绿。
- Cancellation boundary: 受控慢发现测试证明 Stage 返回 `cancelled/130`，ControlService 证明任务终态为 cancelled 且无需 reclaim。生产 heartbeat 间隔为 10 秒，小文件会在取消前结束；本轮没有为制造时间窗口而创建大文件、重扫真实数据或触发仿真。
- Next real Xpeng run gate: 当前 catalog 9 个 Bundle 中没有 source branch 明确为 `feature/CRGVXPFIII-2492-bl01v06-selena-front` 的条目。两个已知 Xpeng 构建 Bundle 都来自 `release/CRGVXPFIII-2430...`；`branch=existing` 的 Bundle 不足以证明 feature/2492。下一次真实提交只能二选一：从当前 feature/2492 工作区重新编译并登记，或导入一个能提供同等来源证据的已有 Selena 文件夹；随后必须确认 Runtime checksum、MatFilter checksum 与该产物匹配，数据仍使用用户 YAML 中的 Xpeng 共享路径。任一条件不满足只允许 validate/dry-run，禁止真实 Cluster 仿真。
...
- 代码与测试已提交为 `2cc9fe8`（Windows/部分成功）和 `1d6374b`（Cluster 逐任务结果）并推送到 `origin/codex/new-branch`；Cluster/控制/Agent 定向回归 `148 passed`，外围核心集合仍为 `151 passed, 1 warning`。
...
- Windows Connector 已按最新代码重建并由下载接口提供：`8,343,568` bytes，SHA-256 `sha256:7f0570936c9358a94b3842ebc1047e7278628019f9d01446c6998eb689fd2a0a`。包中已包含部分成功的逐输入诊断和 Manifest 归档逻辑。
