...
| 2026-07-28 | Linux 发布门禁的跨平台资产授权修复 | `c3a5b4a` 的 Linux release 焦点门禁首次为 `90 passed, 1 failed`；根因不是生产授权状态或 Xpeng 配置，而是 `candidate_asset_binding_ids()` 仅接受 Windows drive/UNC，导致 Linux pytest 的 POSIX 临时资产路径无法映射到已登记根目录。修复只在非 Windows host 接受 POSIX 绝对路径；Windows 仍只接受 drive/UNC、相对路径仍拒绝，且最终文件仍必须在已登记可读根目录内。专项 `23 passed, 1 skipped`、编译和 diff check 均通过；随后必须以独立 `RSIM_HOME` 重跑 Linux release 门禁，禁止清理或修改生产 Agent DB 来获取绿灯。 |
